...
const { listProducts } = require("./product-repository");
...
async function refreshProductCacheFromDatabase() {
  initializationError = undefined;
  initializationPromise = undefined;

  try {
    return await loadProductsCacheFromDatabase();
  } catch (error) {
    initializationError = error;
    throw error;
  }
...
function upsertProductInCache(product) {
  ...
  const existingIndex = products.findIndex(
    (item) => item.short_code === cachedProduct.short_code
  );

  if (existingIndex >= 0) {
    products.splice(existingIndex, 1, cachedProduct);
  } else {
    products.unshift(cachedProduct);
  }
...
module.exports = {
  ...
  refreshProductCacheFromDatabase,
